...
// This component receives the initial session data from the server
export function DashboardClient({ initialSessions }: { initialSessions: SessionWithDetails[] }) {
  ...
  const filteredSessions = React.useMemo(() => {
    if (proficiencyFilter === "all") return initialSessions;
    return initialSessions.filter(s => s.proficiencyLevel === proficiencyFilter);
  }, [proficiencyFilter, initialSessions]);
